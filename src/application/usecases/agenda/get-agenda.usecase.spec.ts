...
describe('GetAgendaUsecase', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    usecase = new GetAgendaUsecase(mockAgendaRepository as any);
  ...
  describe('execute', () => {
    it('should call repository with correct date range for DAY filter', async () => {
      ...
      const [, startDate, endDate] = mockAgendaRepository.getAgenda.mock.calls[0];
    ...
    it('should return the agenda from the repository', async () => {
      ...
      expect(mockAgendaRepository.getAgenda).toHaveBeenCalledWith('cid', expect.any(Date), expect.any(Date));
    });
  });
});

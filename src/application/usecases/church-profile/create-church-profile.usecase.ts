...
import { ChurchProfileRepository, CreateChurchProfileInput } from 'src/domain/repositories/church-profile.repository';
...
export class CreateChurchProfileUsecase {
  ...
  async execute(input: CreateChurchProfileInput): Promise<IChurchProfile> {
    const existing = await this.churchProfileRepository.findByChurchIdIncludingDeleted(input.churchId);
    if (existing && !existing.deletedAt) throw new ConflictException('Church profile already exists');

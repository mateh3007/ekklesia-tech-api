...
describe('UpdatePrayerRequestUsecase', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    usecase = new UpdatePrayerRequestUsecase(mockPrayerRequestRepository as any);
  });

  it('should update prayer request successfully', async () => {
    const pr = { id: 'prid', churchId: 'cid', name: 'João', request: 'Original' };
    ...
    const result = await usecase.execute('prid', { request: 'Atualizado' }, 'cid');

    expect(result).toBe(updated);
    expect(mockPrayerRequestRepository.update).toHaveBeenCalledWith('prid', { request: 'Atualizado' });
  });

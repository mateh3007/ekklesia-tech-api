...
export class AssignChurchPermissionUsecase {
  ...
  async execute(churchId: string, permissionId: string): Promise<IChurchPermission> {
    ...
    const alreadyAssigned = await this.churchPermissionRepository.findByChurchId(churchId);
    const duplicate = alreadyAssigned.some((cp) => cp.permissionId === permissionId);
    if (duplicate) throw new ConflictException('Permission already assigned to this church');

...
export class PrismaChurchInviteRepository extends ChurchInviteRepository {
  ...
  async create(data: CreateChurchInviteInput): Promise<IChurchInvite> {
    return this.prisma.churchInvite.create({ data }) as Promise<IChurchInvite>;
  }

  async findAll(): Promise<IChurchInvite[]> {
    return this.prisma.churchInvite.findMany() as Promise<IChurchInvite[]>;
  }
  ...
  async findById(id: string): Promise<IChurchInvite> {
    ...
    return invite as unknown as IChurchInvite;
  }
  ...
  async findByToken(token: string): Promise<IChurchInvite | null> {
    return this.prisma.churchInvite.findUnique({ where: { token } }) as Promise<IChurchInvite | null>;
  }

  async findPendingByEmailAndChurch(email: string, churchId: string): Promise<IChurchInvite | null> {
    return this.prisma.churchInvite.findFirst({
      where: { email, churchId, status: InviteStatus.PENDING },
    }) as Promise<IChurchInvite | null>;
  }

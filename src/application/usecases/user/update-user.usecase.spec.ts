...
describe('UpdateUserUsecase', () => {
  ...
  it('should update user successfully without changing email or password', async () => {
    ...
    const result = await usecase.execute('user-id', { name: 'New Name' }, 'church-id');
  ...
  it('should allow updating email when new email is not in use', async () => {
    ...
    mockUserRepository.update.mockResolvedValue({ ...user, email: 'new@test.com' });

    const result = await usecase.execute('user-id', { email: 'new@test.com' }, 'church-id');
  ...
  it('should allow updating email to the same email (same user)', async () => {
    ...
    await expect(usecase.execute('user-id', { email: 'user@test.com' }, 'church-id')).resolves.toBeDefined();
  });
  ...
  it('should throw ForbiddenException when user belongs to different church', async () => {
    mockUserRepository.findById.mockResolvedValue(makeUser('user-id', 'other-church'));

    await expect(usecase.execute('user-id', {}, 'church-id')).rejects.toThrow(ForbiddenException);
  });

  it('should throw ConflictException when new email is already in use by another user', async () => {
    const user = makeUser();
    mockUserRepository.findById.mockResolvedValue(user);
    mockUserRepository.findByEmail.mockResolvedValue({ id: 'other-user-id', email: 'taken@test.com' });

    await expect(usecase.execute('user-id', { email: 'taken@test.com' }, 'church-id')).rejects.toThrow(
      ConflictException,
    );
  });
});
